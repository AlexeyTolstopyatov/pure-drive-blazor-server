...
namespace puredrive.Services
{
    public class Deflate
    {
        ...
        public static string FastCompress(object text)
        {
            Deflate deflate = new Deflate();

            if (string.IsNullOrEmpty(text.ToString()))
                text = "no text";
            deflate.Text = text.ToString()!;
            deflate.Compress();

            return deflate.Result;
        }

        public static string FastDecompress(object bytes)
        {
            Deflate deflate = new Deflate(bytes.ToString()!);
            deflate.Decompress();
            return deflate.Result;
        }
